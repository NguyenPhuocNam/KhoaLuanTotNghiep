...
📌 **Link tải Dataset:** [Tải bộ dữ liệu tại đây](https://drive.google.com/drive/folders/15fj6RSgMQreXaqhVfMajF03ecnDjbT1O?usp=drive_link)
...
## ⚙️ 4. Cấu hình huấn luyện (Hyperparameters)
...
Repository bao gồm các file mã nguồn chính yếu sau:
- 📓 `Fine-tuneCCTT.ipynb`: File notebook chứa mã nguồn huấn luyện theo phương pháp chưng cất tri thức (Knowledge Distillation) nhằm đồng bộ không gian vector.
- 📓 `HuanLuyenCombierVaDanhGia.ipynb`: File notebook chứa mã nguồn để huấn luyện hàm Combiner và thực hiện quá trình đánh giá (evaluation) toàn bộ hệ thống.

## 🚀 6. Trọng số mô hình đã huấn luyện (Fine-tuned Models)
Các trọng số (weights) của mô hình sau quá trình fine-tune được lưu trữ trực tuyến. Bạn có thể tải xuống để chạy thử nghiệm hoặc đánh giá:
📌 **Link tải Mô hình:** [Tải các mô hình đã fine-tune tại đây](https://drive.google.com/drive/folders/1XpagYjGWapSKFzO5570jyOwBEY1c1vvl?usp=drive_link)
